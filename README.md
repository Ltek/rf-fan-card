[![Open your Home Assistant instance and open a repository inside the Home Assistant Community Store.](https://my.home-assistant.io/badges/hacs_repository.svg)](https://my.home-assistant.io/redirect/hacs_repository/?owner=LTek&repository=rf-fan-test-card&category=dashboard)

# RF Fan Test Card

A Home Assistant custom **Dashboard** card for bench-testing the RF codes learned by the
[`ha-rf-fan`](https://github.com/dasimon135/ha-rf-fan) integration.
...
(`default_view: remote | test | both`). When both regions are enabled, header chips show/hide each
live; when only one is enabled the chips are hidden.
...
into the editor (or see [`rf-fan-codes.v3.yaml`](rf-fan-codes.v3.yaml) for the full map and a `raw:`
...
1. Copy `rf-fan-test-card.js` to `/config/www/rf-fan-test-card.js`.
2. Add it as a Dashboard resource (Settings → Dashboards → ⋮ → Resources):
   - URL: `/local/rf-fan-test-card.js`
   - Type: **JavaScript Module**
3. Add the card to a dashboard: type **RF Fan Test Card** in the card picker, or use YAML below.
...
[`rf-fan-codes.v3.yaml`](rf-fan-codes.v3.yaml).
...
| `headers` | map | *see notes* | Per-section header visibility: `light` (default hidden), `forward`, `reverse`, `breeze`, `timer` (default shown). |
